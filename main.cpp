...
int main()
{
	...
	Texture texture;
	texture.loadFromFile("/home/vera/Tetris/image/figures.png");
	
	// Создание спрайта
	Sprite sprite(texture);
	
	// Вырезаем из спрайта отдельный квадратик размером 21х21 пикселей
	...
	while (window.isOpen())
	{
		...
		for (int i = 0; i < M; i++)
			for (int j = 0; j < N; j++)
			{
				...
				sprite.setTextureRect(IntRect(field[i][j] * 21, 0, 21, 21));
				sprite.setPosition(j * 21, i * 21);
		...
		for (int i = 0; i < 4; i++)
		{
			...
			sprite.setTextureRect(IntRect(colorNum * 21, 0, 21, 21));
			
			// Устанавливаем позицию каждого кусочка тетрамино
			sprite.setPosition(a[i].x * 21, a[i].y * 21);
